...
def doctree_resolved(app, doctree, docname):
    ...
    if app.builder.name in ('html', 'singlehtml', 'epub'):
        ...
        if sys.version_info < (3, 1):
            envdocname_figs_items = env.docname_figs.iteritems()
        else:
            envdocname_figs_items = env.docname_figs.items()

        for figdocname, figurelist in envdocname_figs_items:
        ...
        for secnum in secnums:
            if secnum[0] != last_secnum:
                figid = 1
            if sys.version_info < (3, 1):
                fignames_by_secnum_items = fignames_by_secnum[secnum].iteritems()
            else:
                fignames_by_secnum_items = fignames_by_secnum[secnum].items()

            for figname, subfigs in fignames_by_secnum_items:
        ...
        for figure_info in doctree.traverse(lambda n: isinstance(n, nodes.figure) or
            ...
            for cap in figure_info.traverse(nodes.caption):
                ...
                if fignum[-1] in map(str, range(10)):
                    boldcaption = "%s %s:" % (
                        app.config.figure_caption_prefix, fignum)
                else:
        ...
        for ref_info in doctree.traverse(num_ref):
            ...
            if target not in docnames_by_figname:
                ...
                if target in figids:
                    ...
                    # linktext = 'aaaaaaaaaaaa'
                    pass
                else:
                    linktext = target
                    # linktext = 'bbbbbbbbbbb'
